...
ihmApp.service('servicoGenerico', function () {
		...
		this.enviaMsg = function (msg, dsup) {
			...
			console.info("{f-enviaMsg}.ini - validacao se pode enviar - versaoIhmWeb: 1.0.1"  );
			...
			console.info(up + "{f-enviaMsg} . versao-ihmweb: versaoIhmWeb: 1.0.1");

			console.info(up + "{f-enviaMsg} . vou tentar enviar msg " + msg);

			if (up != '-1' || logon) {

				console.info("{f-enviaMsg} preparando execucao doSendConnect .x");
				var msgac0 = "ac=0#ihm="+ IP + "#ip=" + IP + "#pt=3005#"
				console.info("{f-enviaMsg} executando doSendConnect .x: " + msgac0);
				doSendConnect(msgac0);
				console.info("{f-enviaMsg} doSendConnect executado .x: " + msgac0);
				
				console.info("{f-enviaMsg} .x");
				console.info("{f-enviaMsg} agora executando doSend .x");
				var websocket = doSend(msg);
				setTimeout(function () {
					if(websocket.readyState == 1){
						console.info("{f-enviaMsg} enviada com sucesso");
						logMsgEnviada(msg);
		...
		this.enviaMsgReprocesso = function (msg, dtoM, $state) {
			...
			console.info(up + "{f-enviaMsgReprocesso} . vou tentar enviar msg " + msg);

			if (up != '-1') {
				doSend(msg);
				console.info(up + "{f-enviaMsgReprocesso} . disparou doSend ");
	...
	.controller('EntrarController', ['$scope', '$state', 'AuthService', '$cookieStore', function ($scope, $state, AuthService, $cookieStore) {
		...
		//$scope.versaoIhmWeb = "1.0.1"; //Versao de acordo com valores do commit no svn.

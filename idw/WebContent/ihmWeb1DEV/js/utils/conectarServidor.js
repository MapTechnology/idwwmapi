...
function conectar() {
	...
	sessaoTimer = setInterval(function pegasessao(){
		if(websocketTime.readyState == 1){
			console.info("{f-conectarServidor}.send - versaoIhmWeb: 1.0.1"  );
			console.info("{f-conectarServidor}.send : " + "ac=0#ihm="+ ip + "#ip=" + ip + "#pt=3005#");
			websocketTime.send("ac=0#ihm="+ ip + "#ip=" + ip + "#pt=3005#");
		}
		else{
			console.info("{f-conectarServidor}.send : instanciaWebSocket" );
			websocketTime = instanciaWebSocket();
		}
	} , 10000);
}
...
function instanciaWebSocket(){
	console.info("conectando na url " + wsUri);
	...
	setTimeout(function () {
					if(novoWebsocket.readyState == 1){
						console.info("instanciaWebSocket enviada com sucesso");
...
function doSend(message) {
	...
	console.info("doSend. websocket.readyState=" + websocket.readyState);

	if(websocket.readyState == 1) {
		websocket.send(message);
		console.info("doSend. enviado com sucesso");
	} else {

...
if(document.all || document.layers || document.getElementById){
	...
	document.write("<script language=\"JavaScript1.2\" src=\""+eyemenu__codebase+"eyemenu_"+brn+".js\"></script>");
...
function q32(index){
	eval(eval("window.eyemenu__clickitem_code"+index));
	if(!eval("window.eyemenu__url"+index))
		return;
	if(eval("window.eyemenu__urltarget"+index))
		where=eval("eyemenu__urltarget"+index);
	else 
		where=eyemenu__urltarget;
	
	if(where.length<1 || where.toLowerCase()=="_self")
		window.location.href=eval("eyemenu__url"+index);
	else {
		if(where.toLowerCase().indexOf("_parent")>-1)
			eval("parent.window.location=eyemenu__url"+index);
		else  if(where.toLowerCase().indexOf("_new")<0)
				eval("parent."+where+".location=eyemenu__url"+index);
			 else 
				window.open(eval("eyemenu__url"+index));
	}
};
function q29(){
...
function q100(){
	...
	q101=0;
	q102=0;
	while(eval("window.eyemenu__subdesc"+q101+"_0"))
		q101++;
	while(eval("window.eyemenu__rollover_image"+q102))
		q102++;
		(q102>q101)? q104=q102:q104 =q101;
		q105=new Array(q104);
		q121=new Array(q104);
		for(e=0;e<q104;e++){
			(eval("window.eyemenu__subdesc"+e+"_0"))? q105[e]=true:q105[e]=false;
			(eval("window.eyemenu__rollover_image"+e))? q121[e]=true:q121[e]=false;
		}
		return q104;
};
function q122(hide,id){
	if(!hide){
		if(!(tval=eval("window.eyemenu__status_text"+id))){
			q115=eval("window.eyemenu__url"+id);
			if((eval("window.eyemenu__show_urls_statusbar"))&&(q115))
				tval=q115;
		}
		if(tval){
			status=tval;

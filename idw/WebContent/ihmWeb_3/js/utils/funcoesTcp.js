...
function construirMensagemNovaOpAserTransmitida(dto , codigo , idPt, tpSessaoObjectInf01, tpSessaoObjectInf02, tpSessaoInteger)
{
	switch (codigo) {
		case NOVA_OP:
			var msg = adicionarCabecalho(NOVA_OP, idPt) + "nrop=" + dto + SEPARADOR
			+ "tpsessao=" + tpSessaoInteger + SEPARADOR;
		break;

		case NOVA_OP_MOLDE:
			var msg = adicionarCabecalho(NOVA_OP, idPt) + "cdmolde=" + dto + SEPARADOR
			+ "tpsessao=" + tpSessaoInteger + SEPARADOR;
		break;

		case NOVA_OP_PRODUTO:
			var msg = adicionarCabecalho(NOVA_OP, idPt) + "cdProduto=" + dto + SEPARADOR
			+ "tpsessao=" + tpSessaoInteger + SEPARADOR;
		break;

		case NOVA_OP_MOLDE_ESTRUTURA:
			var msg = adicionarCabecalho(NOVA_OP, idPt) + "cdmolde=" + dto + SEPARADOR
				+ "cdestrutura=" + tpSessaoObjectInf01 + SEPARADOR
				+ "qtdeciclos=" + tpSessaoObjectInf02 + SEPARADOR
				+ "tpsessao=" + tpSessaoInteger + SEPARADOR;
		break;

		case NOVA_OP_PRODUTO_ESTRUTURA:
			var msg = adicionarCabecalho(NOVA_OP, idPt) + "cdProduto=" + dto + SEPARADOR
				+ "cdestrutura=" + tpSessaoObjectInf01 + SEPARADOR
				+ "qtdeciclos=" + tpSessaoObjectInf02 + SEPARADOR
				+ "tpsessao=" + tpSessaoInteger + SEPARADOR;
		...
		case NOVA_OP_PRODUTO_MONTAGEM:
			var msg = adicionarCabecalho(NOVA_OP, idPt) + "cdProduto=" + dto + SEPARADOR
				+ "tpsessao=" + tpSessaoInteger + SEPARADOR;
		break;

		case NOVA_OP_COM_OPCRIACAOMASTER:
			var msg = adicionarCabecalho(NOVA_OP, idPt) + "cdProduto=" + dto + SEPARADOR
				+ "qtdeciclos=" + tpSessaoObjectInf01 + SEPARADOR
				+ "tpsessao=" + tpSessaoInteger + SEPARADOR;
		break;

		case NOVA_OP_MOLDE_PRODUTO:
			var msg = adicionarCabecalho(NOVA_OP, idPt) + "cdmolde=" + dto + SEPARADOR
				+ "cdProduto=" + tpSessaoObjectInf01 + SEPARADOR
				+ "qtdeciclos=" + tpSessaoObjectInf02 + SEPARADOR
				+ "tpsessao=" + tpSessaoInteger + SEPARADOR;
		break;
	}
...
function construirMensagemAserTransmitida(dto , codigo , idPt) {
	switch (codigo) {
		...
		case CICLO:
			var msg = adicionarCabecalho(codigo , idPt) + "login=" + dto.login + SEPARADOR +
			"dthrlogin=" + dto.dthrLogin + SEPARADOR;
		break;


	case LOGOUT:
		var msg = adicionarCabecalho(LOGOUT , idPt) + "idup=" + idPt + SEPARADOR +
		"login=" + dto.login + SEPARADOR +

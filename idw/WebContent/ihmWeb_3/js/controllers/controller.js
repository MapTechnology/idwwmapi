...
ihmApp.service('servicoGenerico', function () {
	...
	.service('maquinaEspecificaService', [ '$filter', 'servicoGenerico', function($filter, servicoGenerico) {	
		
		this.setNewDadosMaquinaInSession = function(data) {
			var dados = JSON.stringify(data);
			var dadosMaquina;
			var url = "http://" + IP + ":" + PORTA + "/idw/rest/injet/monitorizacao/injettemporealihmweb";
						
			// var data2 = {}
			// data2["cdCp"] = "";
			// data2["cdPosto"] = data.cdPt;
			// data2["dtReferencia"] = $filter('date')(Date.parse(new Date), 'dd/MM/yyyy');
			// data2["filtroOp"] = 0;
			// data2["idTurno"] = 1;
			// data2["tpId"] = 1;
			// data2["tipoPosto"] = 3;
			// var dados2 = JSON.stringify(data2);
			// var dadosMaquina2;			
			// var url2 = "http://" + IP + ":" + PORTA + "/idw/rest/injet/monitorizacao/detalhe";
			...
			.success( function (response) {
				sessionStorage.removeItem('dadosMaquina');					
				dadosMaquina = JSON.parse(response);
				sessionStorage.setItem("dadosMaquina" , JSON.stringify(dadosMaquina));
			})
			.error( function () {
			...
			.always(function () {
				...
				// 		dadosMaquina2 = JSON.parse(response2);
				// 		dadosMaquina.ritmo = dadosMaquina2.indicadores.eficienciaCiclo;	
				// 		sessionStorage.setItem("dadosMaquina" , JSON.stringify(dadosMaquina));					
				// 	},
	...
	.controller('EntrarController', ['$scope', '$state', 'AuthService', '$cookieStore', function ($scope, $state, AuthService, $cookieStore) {
		...
		if ($cookieStore.get('servidor') != '' && $cookieStore.get('porta') != '')
		{
		...
		$scope.entrar = function () {
			...
			$state.go('ihmWeb.selecaoMaquina'); // primeira tela a aparecer depois de logar 
			$scope.$apply();
		}
	...
	.controller('NavbarController', ['$scope', '$state', '$stateParams', 'maquinaEspecificaService', '$timeout', function ($scope, $state, $stateParams, maquinaEspecificaService, $timeout) {
		$scope.maquina = $stateParams.maquina;
		$scope.sessao = Sessoes_G;
		$scope.loading = false;

		if($scope.maquina != null || $scope.maquina != undefined){
			sessionStorage.setItem('cdSelecionado', $scope.maquina);
		}

		$scope.cdSelecionado = sessionStorage.getItem('cdSelecionado');

		//Atualizar gráfico e variaveis da máquina na tela de MaquinaEspecifica
		$scope.updateGraphic = function() {		
			$scope.sessao = Sessoes_G;
			$scope.loading = true;
			$scope.cdSelecionado = sessionStorage.getItem('cdSelecionado');

			$scope.prodRealizada = 0;
			$scope.prodPlanejada = 0;
			$scope.prodFaltante = 0;

			for(let i=0; i<$scope.sessao[$scope.cdSelecionado].pt.listaProducao.length; i++){
				$scope.prodRealizada = parseFloat($scope.prodRealizada) + parseFloat($scope.sessao[$scope.cdSelecionado].pt.listaProducao[i].producaorealizada);
			}

			for(let i=0; i<$scope.sessao[$scope.cdSelecionado].pt.listaProducao.length; i++){
				$scope.prodPlanejada = parseFloat($scope.prodPlanejada) + parseFloat($scope.sessao[$scope.cdSelecionado].pt.listaProducao[i].producaoplanejada);
			}

			for(let i=0; i<$scope.sessao[$scope.cdSelecionado].pt.listaProducao.length; i++){
				$scope.prodFaltante = parseFloat($scope.prodPlanejada) - parseFloat($scope.prodRealizada );

				if($scope.prodFaltante < 0){
					$scope.prodFaltante = 0;
				}
			}
			...
			var data = {};
			data["idIHM"] = "ihmweb3";
			data["cdPt"] = $scope.sessao[$scope.cdSelecionado].pt.dsup;

			maquinaEspecificaService.setNewDadosMaquinaInSession(data).then( function(response) {
				$scope.dadosMaquina = JSON.parse(sessionStorage.getItem("dadosMaquina"));
				var oee = 0, disponibilidade = 0, ritmo = 0, qualidade = 0;

				if($scope.dadosMaquina){
					oee = parseFloat($scope.dadosMaquina.oee.length > 0 ? $scope.dadosMaquina.oee : 0);
					disponibilidade = parseFloat($scope.dadosMaquina.disponibilidade > 0 ? $scope.dadosMaquina.disponibilidade : 0);
					ritmo = parseFloat($scope.dadosMaquina.ritmo > 0 ? $scope.dadosMaquina.ritmo : 0);
					qualidade = parseFloat($scope.dadosMaquina.qualidade > 0 ? $scope.dadosMaquina.qualidade : 0);	
				}
								
				var valuesOee = '0:' + (oee > 100? oee: 100) + ':2';
				var valuesDisponibilidade = '0:' + (disponibilidade > 100? disponibilidade: 100) + ':2';
				var valuesRitmo = '0:' + (ritmo > 100? ritmo: 100) + ':2';
				var valuesQualidade = '0:' + (qualidade > 100? qualidade: 100) + ':2';

				$scope.inicioPlanejado = $scope.dadosMaquina.dtHrIniPlan;
				$("#inicioPlanejado").html($scope.dadosMaquina.dtHrIniPlan);
				if($scope.inicioPlanejado.length == 0)					
					$scope.inicioPlanejado = "--/--/--";

				$scope.inicioReal = $scope.dadosMaquina.dtHrIniReal;
				$("#inicioReal").html($scope.dadosMaquina.dtHrIniReal);
				if($scope.inicioReal.length == 0)					
					$scope.inicioReal = "--/--/--";

				$scope.finalPlanejado = $scope.dadosMaquina.dtHrFimPlan;
				$("#finalPlanejado").html($scope.dadosMaquina.dtHrFimPlan);
				if($scope.finalPlanejado.length == 0)					
					$scope.finalPlanejado = "--/--/--";

				$scope.finalPrevisto = $scope.dadosMaquina.dtHrFimPrev;
				$("#finalPrevisto").html($scope.dadosMaquina.dtHrFimPrev);
				if($scope.finalPrevisto.length == 0)					
					$scope.finalPrevisto = "--/--/--";
						
				zingchart.render({
					id: 'oee',
					data: configZingChart(oee, valuesOee),
					height: 300,
					width: '100%'
				});
				...
				zingchart.render({
					id: 'disponibilidade',
					data: configZingChart(disponibilidade, valuesDisponibilidade),
					height: 300,
					width: '100%'
				});
				...
				zingchart.render({
					id: 'ritmo',
					data: configZingChart(ritmo, valuesRitmo),
					height: 300,
					width: '100%'
				});
				...
				zingchart.render({
					id: 'qualidade',
					data: configZingChart(qualidade, valuesQualidade),
					height: 300,
					width: '100%'
				});
		...
		//Responsável por identificar e mostrar opções(botões ou descrição) no menu horizontal, conforme o state ou pagina aberta.
		$scope.carregarVariaveis = function(){
			
			switch ($state.current.name) {
				case 'ihmWeb.maquinaEspecifica':
					$scope.inicioss = true;
					break;
				case 'ihmWeb.operadores':
					$scope.operador = true;
					break;
				case 'ihmWeb.producao':
					$scope.fnlpross = true;
					break;
				case 'ihmWeb.refugo':
					$scope.aptRss = true;
	...
	.controller('SelecaoMaquinaController', ['$scope', 'servicoGenerico', '$state', '$timeout', function ($scope, servicoGenerico, $state, $timeout) {
		$scope.sessao = Sessoes_G;	
		$scope.loading = false;
		...
		$scope.updateGraphic = function() {
			$scope.loading = true;
			$scope.sessao = Sessoes_G;
			$scope.carregaGrafico().then( function(data){
				$scope.loading = false;
				$scope.$apply();				
			});
		}
		...
		$scope.carregaGrafico = function () {
			...
			return new Promise(function(resolve, reject) {
				...
				for(let i=0; i<$scope.sessao.length; i++){
					
					var data = {};
					data["idIHM"] = "ihmweb3";
					data["cdPt"] = $scope.sessao[i].pt.dsup;
					...
					$.ajax({
						...
						dataType: 'text',
						success: function (response) {					
							$scope.dadosMaquina = JSON.parse(response);		
							var oee = 0;	
							...
							zingchart.render({
								id: idImg,
								data: configZingChartSl(oee, valuesOee),
								height: 170,
								width: "100%"
							});
		...
		$scope.iniciarTela = function(indexClicado){
			if($scope.sessao[indexClicado].pt.producao.ordem == '-1'){
				$state.go('ihmWeb.iniciaopinjet', {maquina: indexClicado});
			}else{
				$state.go('ihmWeb.maquinaEspecifica', {maquina: indexClicado});
				sessionStorage.setItem('inicio', "sim");
			}
		}
	...
	.controller('SuspenderProducaoController', ['$scope', 'servicoGenerico','$stateParams',  function ($scope, $stateParams, servicoGenerico) {
		$scope.sessao = Sessoes_G;
	}])

	.controller('MaquinaEspecificaController', ['$scope', 'servicoGenerico', 'maquinaEspecificaService', function ($scope, servicoGenerico, maquinaEspecificaService) {
		$scope.sessao = Sessoes_G;
		$scope.cdSelecionado = sessionStorage.getItem('cdSelecionado');
		...
		$scope.updateGraphic = function() {
			console.log("entrou e ativou refresh");
		...
		$scope.lancaCiclo = function () {
			...
			if (IsIDWAtivo == "true") {
				...
				$.ajax({
					...
					timeout: 30000,
					success: function (response) {
						$state.go('ihmWeb.maquinaEspecifica', { dto: $scope.dto , selecMaquina : true});
	...
	.controller('FinalizaOpController', ['$scope', 'servicoGenerico', '$stateParams', '$state', function ($scope, servicoGenerico, $stateParams, $state) {
		$scope.sessao = Sessoes_G;
		$scope.dsup = $stateParams.pt;
		$scope.ordem = $stateParams.ordem;
		...
		$scope.finalizaOp = function () {
			...
			setTimeout(function () {$state.go('ihmWeb.selecaoMaquina')}, 1000);
		}
	...
	.controller('IniciaOpControllerInjet', ['$scope', 'servicoGenerico', function ($scope, servicoGenerico) {
		...
		$scope.cdSelecionado = sessionStorage.getItem('cdSelecionado');
		$scope.dsup_sel = $scope.sessao[$scope.cdSelecionado];
		$scope.tpSessaoInteger = $scope.dsup_sel.pt.tpSessao;
		montaStringpTSessao($scope.tpSessaoInteger);

		$scope.montaStringTpSessao =  function (tpSessaoInteger) {
			montaStringpTSessao(tpSessaoInteger);
	...
	.controller('ParadasController', ['$scope', 'servicoGenerico', '$stateParams', '$filter', function ($scope, servicoGenerico, $stateParams, $filter) {
		montaStringParadas();
		$scope.sessao = Sessoes_G;
		$scope.maquina = $stateParams.maquina;
		...
		$scope.sessao[$scope.cdSelecionado].pt.parada.inipFormatado = $filter('date')(Date.parse($scope.sessao[$scope.cdSelecionado].pt.parada.inip), 'dd/MM/yyyy HH:mm:ss');
		...
		$scope.carregaGraficos = function () {
			...
			var oee = parseFloat($scope.dadosMaquina.oee);
			var valuesOee = '0:' + (oee > 100? oee: 100) + ':2';

			var qualidade = parseFloat($scope.dadosMaquina.qualidade);
			var valuesQualidade = '0:' + (qualidade > 100? qualidade: 100) + ':2';

			var ritmo = parseFloat($scope.dadosMaquina.ritmo);
			var valuesRitmo = '0:' + (ritmo > 100? ritmo: 100) + ':2';

			var disponibilidade = parseFloat($scope.dadosMaquina.disponibilidade);
			var valuesDisponibilidade = '0:' + (disponibilidade > 100? disponibilidade: 100) + ':2';
	...
	.controller('AlertasController', ['$scope', 'servicoGenerico', '$filter', function ($scope, servicoGenerico, $filter) {
		montaStringAlertas();
		$scope.alertas = [];
		$scope.dusp = "";
		$scope.cdalerta = "";
		...
		$scope.sessao[$scope.cdSelecionado].pt.listaAlertas.forEach(function cadaPt(element, index, array) {			
			element.dthralertaFormatada = $filter('date')(Date.parse(element.dthralerta), 'dd/MM/yyyy HH:mm:ss');
			$scope.alertas.push(element);			
		});
	...
	.controller('RefugoController', ['$scope', 'servicoGenerico', '$filter', function ($scope, servicoGenerico, $filter) {
		montaStringRefugos();
		$scope.sessao = Sessoes_G;
		...
		$scope.cdSelecionado = sessionStorage.getItem('cdSelecionado');
		element = $scope.sessao[$scope.cdSelecionado];
		...
		if (element.pt.refugo.dtref != null && element.pt.refugo.dtref != "null" && element.pt.refugo.cdrefugo != "-1"
			...
			element.pt.refugo.dtrefFormatada = $filter('date')(Date.parse(element.pt.refugo.dtref), 'dd/MM/yyyy HH:mm:ss');
	...
	.controller('OperadoresController', ['$scope', 'servicoGenerico', '$filter', function ($scope, servicoGenerico, $filter) {
		...
		$scope.sessao[$scope.cdSelecionado].pt.listaOperadores.forEach(function pegaOper(elementOper, indexOper, arrayOper) {
			elementOper.dthrFormatada = $filter('date')(Date.parse(elementOper.dthrLogin), 'dd/MM/yyyy HH:mm:ss');
	...
	.controller('NovoAlertaController', ['$scope', 'servicoGenerico', function ($scope, servicoGenerico) {
		...
		$scope.cdAlertaTText = null;
		$scope.respostaConsultaAlerta;
		...
		$scope.pesquisaAlerta = function () {
			jqxhr = $.get("http://" + IP + ":" + PORTA + "/idw/rest/injet/pts/iwsAlertaDTO/" + $scope.cdAlertaT, 
			function (success, textStatus, xhr) {
				servicoGenerico.respostaGenericaAguarde(false);
				if (success == null || xhr.status != 200) {
					...
					$scope.cdAlertaTText = (success.cdAlerta + " - " + success.dsAlerta);
				}
			})
			.fail(function (error) {
				logConsoleLog(error);
	...
	.controller('RemoveAlertaController', ['$scope', 'servicoGenerico', '$stateParams', function ($scope, servicoGenerico, $stateParams) {
		...
		montaStringRemoveAlerta($scope.cdAlerta);
		$scope.sessao = Sessoes_G;
		$scope.btnRemAlerta = REMOVER_ALERTA_S;
		...
		$scope.removeAlerta = function () {
			...
			dsup = $scope.sessao[$scope.cdSelecionado].pt.dsup;
			var msg = construirMensagemAserTransmitida($scope.cdAlerta, REMOVE_ALERTA, dsup);
	...
	.controller('ApagaRefugoController', ['$scope', 'servicoGenerico', '$stateParams', function ($scope, servicoGenerico, $stateParams) {
		...
		$scope.apagaRefugo = function () {
			var msg = construirMensagemAserTransmitida($scope.sessao[$scope.cdSelecionado].pt.refugo, APAGAULTIMOREFUGO, $scope.sessao[$scope.cdSelecionado].pt.dsup);
			servicoGenerico.enviaMsg(msg,$scope.sessao[$scope.cdSelecionado].pt.dsup);
		}
	...
	.controller('LogoutController', ['$scope', 'servicoGenerico', '$stateParams', '$state', function ($scope, servicoGenerico, $stateParams, $state) {
		$scope.sessao = Sessoes_G;
		...
		$scope.logoutOperador = function () {
			...
			dsup = $scope.sessao[$scope.cdSelecionado].pt.dsup;
			$scope.sessao[$scope.cdSelecionado].pt.listaOperadores.splice($scope.sessao[$scope.cdSelecionado].pt.listaOperadores.indexOf($scope.operador.nome),1);

			var msg = construirMensagemAserTransmitida($scope.operador, LOGOUT, dsup);
			servicoGenerico.enviaMsg(msg, dsup);

			setTimeout(function () {$state.go('ihmWeb.operadores') }, 1000);
		}
	...
	.controller('LoginController', ['$scope', 'servicoGenerico', '$state', '$filter', function ($scope, servicoGenerico, $state, $filter) {
		montaStringLogin();
		$scope.sessao = Sessoes_G;
		$scope.cdSelecionado = sessionStorage.getItem('cdSelecionado');
		$scope.dsup_sel = $scope.sessao[$scope.cdSelecionado];
				...
				$.ajax({
					...
					success: function (response) {
						if (response!= null && response != "-1") {
							...
							op              = OperadoresDTO();
							op.login        = $scope.data.cdUsr;
							op.nome         = $scope.data.cdUsr;
							op.dthrLogin    = $filter('date')(new Date(), 'yyyy-MM-dd HH:mm:ss');
							op.duracaoLogin = '00:00:00';
						
							$scope.sessao[$scope.cdSelecionado].pt.listaOperadores.push(op);
				...
				jqxhr = $.get("http://" + IP + ":" + PORTA + "/idw/rest/refugo/pesquisaRefugo/" + $scope.cdRefugo + "-" +
					...
					, function (success) {
						if ($.isEmptyObject(success) || success.pesquisas.length == 0) {
							...
							$scope.cdRefugoText = (success.pesquisas[0].codigo + " - " + success.pesquisas[0].descricao);
							$scope.$apply();
	...
	.controller('CorrigeUltimaParadaController', ['$scope', 'servicoGenerico', '$stateParams', '$state', function ($scope, servicoGenerico, $stateParams, $state) {
		$scope.sessao = Sessoes_G;
		$scope.pt = $stateParams.pt;
		$scope.acao = false;
		$scope.causa = false;
			...
			var jqxhr = $.get(("http://" + IP + ":" + PORTA + "/idw/rest/injet/paradas/" + "pesquisaParadaCompleta/" + $scope.cdParada + "-" +
				...
				.done(function (success) {
					...
					if ($.isEmptyObject(success)) {
						...
						if (Array.isArray(success.pesquisas)) {
							pesquisa = $scope.verificaPesquisa(success);
							console.log("sucesso e verificaPesquisa", pesquisa);
							if (pesquisa == 0) {
								servicoGenerico.respostaGenerica(VERIFICAR_DADOS_S, FALHA_CLASSE);
								esconderCampos();
								return;
		...
		$scope.corrigirUltimaParada = function () {
			...
			setTimeout(function () {$state.go('ihmWeb.paradas') }, 1000);
		}
		$scope.voltar = function () {
			window.history.back();
		}
...
function atualizaSessaoNB() {
	scopesNB.atualiza();
}

function configZingChart(valor, limites){
	...
	var myConfig = {
		...
		series: [{
			...
			indicator:[5,5,5,5,0.60], //espessura da agulha
			animation: {
				effect: 2,
				method: 1,
				sequence: 4,
...
function configZingChartSl(valor, limites){
	...
	var myConfig = {
		...
		series: [{
			values: [valor], // starting value
			backgroundColor: 'black',
			indicator:[2,2,2,2,0.40], //espessura da agulha
			animation: {
				effect: 2,
				method: 1,
				sequence: 4,

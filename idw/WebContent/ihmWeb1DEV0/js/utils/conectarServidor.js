...
function instanciaWebSocket(){
	console.error("conectando na url " + wsUri);
	...
	setTimeout(function () {
					if(novoWebsocket.readyState == 1){
						console.error("instanciaWebSocket enviada com sucesso");
...
function onOpen(evt) {
	console.error("onOpen: call");
	console.error("onOpen: calling pbConectar(30)");
	pbConectar(30);
	console.error("onOpen: call doSendConnect");
...
function doSendConnect(message){
	...
	console.error("doSendConnect: call");
	console.error("doSendConnect: message=" + message);
	
	console.error("doSendConnect. websocketTime.readyState=" + websocketTime.readyState);

	if(websocketTime.readyState == 3){
		
		console.error("doSendConnect: websocketTime.readyState == 3. call instanciaWebSocket()");

		websocketTime = instanciaWebSocket();

	}else if(websocketTime.readyState == 1){
		
		console.error("doSendConnect: websocketTime.readyState == 1. call websocketTime.send(message)");
...
function doSend(message) {
	...
	console.error("doSend: call");
	
	console.error("doSend: message=" + message);

	var websocket = instanciaWebSocket();
	
	console.error("doSend. websocket.readyState=" + websocket.readyState);

	if(websocket.readyState == 1) {
		websocket.send(message);
		console.error("doSend. enviado com sucesso");
	} else {

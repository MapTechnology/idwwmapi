...
ihmApp.service('servicoGenerico', function () {
		...
		this.enviaMsg = function (msg, dsup) {
			...
			if (up != '-1' || logon) {
				console.info("{f-enviaMsg} executando doSendConnect... .x");
				doSendConnect(msg);
				console.info("{f-enviaMsg} executando doSend... .x");
				...
				if (isoffline) {
					console.error("{f-enviaMsg} isoffline=true. por isso não enviado.");
					window.history.back();

...
//    blockNr = makeword(tftpP[IX_BLOCKNR], tftpP[IX_BLOCKNR+1]);
   ...
   private int getLength()
   {
      ...
      int porta = options.getPorta();
      if (porta > 0) length += options.PORTA.length() + ((String) options.get(options.PORTA)).length() +2;
   ...
   /**
	 * @see tftp.common.TFTPPacket#getBytes()
	 */
   @SuppressWarnings("static-access")
   @Override
   ...
   int IX_OPTION = IX_OPCODE +2;
    ...
    if (porta > 0) 
    {
      int length = options.PORTA.length();
      System.arraycopy(options.PORTA.getBytes(), 0, tftpP, IX_OPTION, length);
      IX_OPTION += length;
      tftpP[IX_OPTION++] = 0;
      
      String portaValue = (String) options.get(options.PORTA);
      length = portaValue.length();
      System.arraycopy(portaValue.getBytes(), 0, tftpP, IX_OPTION, length);
      IX_OPTION += length;
      tftpP[IX_OPTION++] = 0;
    }
  ...
  static public final int OPCODE  = 6;
   /**
    * @see tftp.common.TFTPPacket#getOpCode()
    */
  @Override

...
{  
   ...
   {	   
	 ...
	 * tftp packet.
	 */
	public FRQ(byte[] tftpP,IdwLogger log) throws InstantiationException
	{
		super(tftpP);
		this.log=log;
	 ...
	 * client applications.
	 * 
	 * implement the options extension in tftp for client usage
	 * 
	...
	public byte[] getBytes()
	{
      ...
      if (hasOptions() && tftpOptions.hasOption(TFTPOptions.PORTA))
      {
         optionLength = TFTPOptions.PORTA.length();
         System.arraycopy(
            TFTPOptions.PORTA.getBytes(),
            0,
            tftpP,
            IX_OPTION,
            optionLength);
         ...
         optionValue = "" + tftpOptions.getPorta();
         optionLength = optionValue.length();
         System.arraycopy(
            optionValue.getBytes(),
            0,
      ...
      if (hasOptions() && tftpOptions.hasOption(TFTPOptions.AUX))
      {
         optionLength = TFTPOptions.AUX.length();
         System.arraycopy(
            TFTPOptions.AUX.getBytes(),
            0,
            tftpP,
            IX_OPTION,
            optionLength);
         ...
         optionValue = tftpOptions.getAuxiliar();
         optionLength = optionValue.length();
         System.arraycopy(
            optionValue.getBytes(),
            0,
   ...
   {
	   log.info("Set FileName:"+fileName);
   ...
   public void setTransferSize(int transferSize)
   {
	...
	{
		if (hasOptions())
			return tftpOptions.getAuxiliar();
		return null;
	}
  ...
  {
     if (tftpOptions == null) tftpOptions = new TFTPOptions();
     tftpOptions.setAuxiliar(auxiliar);
  }

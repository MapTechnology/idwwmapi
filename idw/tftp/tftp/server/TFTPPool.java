...
public class TFTPPool
{
	...
	public TFTPPool(int size, Hashtable connections, VirtualFileSystem vfs, EventListener listener,IdwLogger log)
    {
    	this.log=log;
        this.vfs = vfs;
        ...
        //agent =ManagementFactory.getPlatformMBeanServer();
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            try
            {
                wt = new TFTPRHThread(birthDate, new TFTPRequestHandler(vfs, listener,log));
                wt.start();
                idleWorkers.push(wt);
            ...
            {
                log.info(birthDate + " is not a valid ObjectName!", e);
            }
            catch (InstanceAlreadyExistsException e)
            {
                log.info("An instance of " + birthDate + " already exists in agent!", e);
            }
            catch (MBeanRegistrationException e)
            {
                log.info(birthDate + " could not be registered in agent!", e);
            }
            catch (NotCompliantMBeanException e)
            {
                log.info(birthDate + " does not comply to the Java Management Extensions Intrumentation and Agent Specification, v1.1!", e);
            }
            /*         catch (MBeanException e)
                     {
                        tftplog.info("could not register worker thread as XMBean with JMX agent");
                        e.printStackTrace();
                     } */
            catch (SocketException e)
            {
                log.info("Could not create TFTPRequestHandler nr: " + i + " for TFTPPool");
            }
        }
    }
	...
	synchronized public void performWork(FRQ frq, InetAddress clientAddress, int clientPort)
    {
        ...
        if (existingConnection != null)
        {
            ...
            {
                log.info("Hey, I am already working for you. Be patient!");
                return;
            }
        }
        ...
        synchronized (idleWorkers)
        {
            if (idleWorkers.empty())
            {
                ...
                try
                {
                    log.info("WARNING: Overload on tftpPool! ReHash!");
                    wt = new TFTPRHThread(birthDate, new TFTPRequestHandler(vfs, listener,log));
                    wt.start();
    ...
    // implements XMBeanConstants
    {
        ...
        synchronized public void die()
        {
            ...
            {
                log.info(on + " is not a valid ObjectName!", e);
            }
            catch (InstanceNotFoundException e)
            {
                log.info("An instance of " + on + " could not be found in agent!", e);
            }
            catch (MBeanRegistrationException e)
            {
                log.info(on + " could not be unregistered in agent!", e);
            }

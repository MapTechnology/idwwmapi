...
public class TipoBAntecipaSeDer {
	/*
	...
	public SortedMap<IdCtDTO, CtDTO> geraCpsAntecipandoQuandoDer(IdwLogger log, int idLog, int identacao, PpPlano ppplano, OmUsr omusr, SortedMap<IdCtDTO, CtDTO> listaParaProcessamento){
		...
		for (IdCtDTO idctdto : listaParaProcessamento.keySet()){
			...
			for (PassosDTO p : listaordenada){
				...
				// ficara errado, visto que essse passo naturalmente será incorporado no passo anterior
				...
				// pois nesse tempo que se irá adiantar ou nao a producao
				if (pAnterior != null) {
		...
		for (PassosDTO p : lista){
			...
			if (isEncontrouPasso == true){
				if (p.getOmproduto().getOmproduto().getIdProduto() == passo.getOmproduto().getOmproduto().getIdProduto()){
					// Se o passo que sera antecipado NAO tiver predecessora e se as predecessoras
					// que ele tiver tiverem data final anterior a data inicial que o passo sera antecipado
					// entao inclui-lo para ser antecipado
					PassosDTO pPredecMaior = null;
					for (PassosDTO pPredec : p.getPassosPredecessoras()){
						if (pPredec.isExcluida() == true)
							continue;
						
						if (pPredecMaior == null)
							pPredecMaior = pPredec;
						if (DataHoraRN.after(pPredec.getFim(), pPredecMaior.getFim()))
							pPredecMaior = pPredec;
					}
					if (p.getPassosPredecessoras().isEmpty() == true || pPredecMaior == null || DataHoraRN.after(passo.getFim(), pPredecMaior.getFim())) {
						retorno.add(p);

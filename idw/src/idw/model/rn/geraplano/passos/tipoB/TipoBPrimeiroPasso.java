...
public class TipoBPrimeiroPasso {

	private DAOGenerico dao;
	
	public TipoBPrimeiroPasso(DAOGenerico dao){
		this.dao = dao;
	}
		...
		for (PpPlanec ppplanec : listaPpPlanec) {
			...
			for (PpNeccron ppneccron : listaNeccron){
				...
				for (DwRotapasso passo : ultimosPassos){
					...
					if (omprodutoSemiacabado == null)
						continue;
					
					if (omprodutoSemiacabado.getCdProduto().equals("0")){
						if (folhaSemPrograma.contains(passo.getDwFolha().getCdFolha()) == false){
							retorno.setMensagem(retorno.getMensagem() + "A Folha " + passo.getDwFolha().getCdFolha() + " está com o produto 0 na aba Programa IAC. Plano para " + omproduto.getCdProduto() + ".\n");
					...
					// debugar pq o produto 511907 nao considerou o leadtime de 24h ja que nao tem definicao
					
					int leadTime = 0;
					if (ppnec.getHrLeadtime() != null && ppnec.getHrLeadtime().equals(BigDecimal.ZERO) == false){ // Se a necessidade tiver leadtime o mesmo terá prioridade em relacao ao leadtime do produto
						...
						if (omproduto.getHrLeadtimesaida() != null && omproduto.getHrLeadtimesaida().equals(BigDecimal.ZERO) == false) {
							leadTime = omproduto.getHrLeadtimesaida().intValue();
							log.info(idLog, identacao, "Usando o lead-time " + leadTime + " definido no produto");
						} else {
							leadTime = 24; // Alessandre em 11-2-13 mudei o default de 0 para 24h
					...
					planeccron.setQtNecessaria( new BigDecimal(qtNecessaria) ); // A quantidade é a quantidade desejada pelo cliente * a quantidade do semi-produto na estrtuura do MODELO

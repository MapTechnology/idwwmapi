...
public class Cor {
	...
	private static Color[] colorPool = {
		...
		new Color(0xac, 0xc2, 0xd9), // cloudy
	...
	public static String transformarParaCodigoHexadecimal(int rgb) {
		Color cor = new Color(rgb);
		return String.format("#%02x%02x%02x", cor.getRed(), cor.getGreen(),
				cor.getBlue());
	}
	
	public static String transformarParaCodigoHexadecimal(Color cor) {
		if (cor == null) {
			return "";
		}
	...
	public static String transformarColorParaCodigoHexadecimal(String corString) {
		if (corString == null) {
			return "";
		}

		Color cor = new Color(Integer.parseInt(corString));
		return String.format("#%02x%02x%02x", cor.getRed(), cor.getGreen(),
				cor.getBlue());
	}

	public static List<Color> getListaDeCores(int quantidadeDeCores) {
		List<Color> cores = Arrays.asList(colorPool);

		if (cores.size() >= quantidadeDeCores) {
			return cores;
		}

		return getListaRandomicaDeCores(quantidadeDeCores);
	}

	public static List<Color> getListaRandomicaDeCores(int quantidadeDeCores) {
		List<Color> cores = new ArrayList<Color>();

		if (quantidadeDeCores == 0) {
			return cores;
		}

		for (int i = 0; i < 360; i += 360 / quantidadeDeCores) {
			float hue = i;
			float saturation = 90 + (float) Math.random() * 10;
			float brightness = 90 + (float) Math.random() * 10;

			cores.add(new Color(Color.HSBtoRGB(hue, saturation, brightness)));
		}
		return cores;
	}

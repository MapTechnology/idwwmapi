...
public class TrataArquivoRNICtsAte extends TrataArquivoRN {

	private BufferedEventos bufferEvento;
	private List<String> linhasUteis = new ArrayList<String>();
	
	public TrataArquivoRNICtsAte(String pathRelativo, IcUpDTO icupdto, ColetaFileType fileType, IdwLogger log, int idLog) {
	...
	protected boolean processaUmaLinhaDoArquivoCustom(File file, String name, int lineCount, String linhaArquivo) throws PararDeProcessarArquivoSemSalvarLinhaException {
		if (linhaArquivo.contains("Usuário") || linhaArquivo.contains("Data") || linhaArquivo.contains("Hora")){
			linhasUteis.add(0, linhaArquivo);
		}	
			
		if (linhaArquivo.contains("DUT") && ( linhaArquivo.contains("PASS") || linhaArquivo.contains("FAIL") ) ) {
			linhasUteis.add(1,linhaArquivo);
		}
		
		if (linhasUteis.size() == 2){
		
			LinhaArquivoIctsAte linha = new LinhaArquivoIctsAte(log, icupdto, linhasUteis, file.toString());

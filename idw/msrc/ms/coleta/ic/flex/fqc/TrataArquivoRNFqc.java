...
public class TrataArquivoRNFqc extends TrataArquivoRN {

	private BufferedEventos bufferEvento;
	
	public TrataArquivoRNFqc(String pathRelativo, IcUpDTO icupdto, ColetaFileType fileType, IdwLogger log, int idLog) {
	...
	protected boolean processaUmaLinhaDoArquivoCustom(File file, String name, int lineCount, String linhaArquivo) throws PararDeProcessarArquivoSemSalvarLinhaException {
		LinhaArquivoFlexFqc linha = new LinhaArquivoFlexFqc(log, icupdto, linhaArquivo, file.toString());
		List<EventoColetado> ev = linha.obtemEvento();
		if (ev.isEmpty() == false) {
			
			// Alessandre em 21-01-19 comentei a linha abaixo, pois no mommento o ICThread não está salvando o evento qdo se perde conexao com o banco, acarretanto a perda do mesmo
			bufferEvento.addEventos(ev);
			
			// Lancando o evento diretamente caso dê erro no banco a linha será reprrocessada na proxima vez
//			EventoColetado evento = ev.get(0);
//			lancaTesteComDefeito(linhaArquivo, evento.getDthrEvento(), evento.getCb(), 0, evento.getCddefeito());

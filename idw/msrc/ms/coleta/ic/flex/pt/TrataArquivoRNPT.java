...
public class TrataArquivoRNPT extends TrataArquivoRN {
	...
	public TrataArquivoRNPT(String pathRelativo, IcUpDTO icupdto, ColetaFileType fileType, IdwLogger log, int idLog) {
	...
	protected boolean processaUmaLinhaDoArquivoCustom(File file, String name, int lineCount, String linhaArquivo) throws PararDeProcessarArquivoSemSalvarLinhaException {
		log.info("processaUmaLinhaDoArquivoPT" + lineCounter + " - " + linhaArquivo);
		if (lineCounter == 1) {
	...
	private void lancaMontagemMac(String linha, Date horaPassagem, String serialNumber, String mac, boolean stTeste, String cddefeito) {
		...
		try {
			...
			IdwFacade.getInstancia().registrarMontagem(cdUp, null, serialNumber, horaPassagem, montagens, "1", stTeste, cddefeito);
			Thread.sleep(500);
	...
	private void processaUmaLinhaDoArquivoPK(String linhaArquivo) {
		...
		try {
			fimCicloDate = format.parse(date + " " + endTime);
			if (errorCode.equalsIgnoreCase("T908"))
				lancaMontagemMac(linhaArquivo, fimCicloDate, serialNumber, mac, true, errorCode);
			else

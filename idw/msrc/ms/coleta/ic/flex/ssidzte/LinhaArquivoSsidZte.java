...
public class LinhaArquivoSsidZte {
	...
	public LinhaArquivoSsidZte(IdwLogger log, IcUpDTO icUpDTO, String linha,String nomeArquivo) {
		this.log = log;
	...
	protected EventoColetado processarLinha() {
		...
		mac = linhaSplitada[2];
		dateTime = linhaSplitada[7] + " " + linhaSplitada[8];
	...
	private EventoColetado criaEventoColetado(String cdUp, String cdOp, String string, Date dataHoraFimTeste, String qtde) {
		...
		montagens.add(createMontagemSerialPlaca());
		ev.setMontagem(montagens);
		
		return ev;
	}
	private MontagemDTO createMontagemSerialPlaca() {
	...
	public List<EventoColetado> obtemEvento() {
		...
		try {
			...
			log.info("LinhaArquivoSsidZte: Excessao em obtemEvento da maquina: "

...
public class TrataArquivoRNForno1809 extends TrataArquivoRN {

	private BufferedEventos bufferEvento;
	
	public TrataArquivoRNForno1809(String pathRelativo, IcUpDTO icupdto, ColetaFileType fileType, IdwLogger log, int idLog) {
		...
		protected boolean isMesmoArquivoEncontrouUltimaLinha(RandomAccessFile reader, String arquivoProcessado) throws IOException {
			...
			for (int i = 0; i < lineCounter; i++) {
				...
				if (linhaArquivo != null &&
						...
						linhaArquivo.contains(this.ultimaLinhaProcessada.substring(0, ultimaLinhaProcessada.length()-10))) {
					return true;
				}
			}
			...
			boolean isRetorno = (linhaArquivo != null &&
					this.ultimaLinhaProcessada != null &&
					linhaArquivo.equals("") == false &&
					linhaArquivo.contains(this.ultimaLinhaProcessada.substring(0, ultimaLinhaProcessada.length()-10))) ? true : false;
	...
	protected boolean processaUmaLinhaDoArquivoCustom(File file, String name, int lineCount, String linhaArquivo) throws PararDeProcessarArquivoSemSalvarLinhaException {
		//LinhaArquivoForno linha = new LinhaArquivoForno(log, icupdto, linhaArquivo, file.toString());
		LinhaArquivoForno linha = new LinhaArquivoForno(log, icupdto, linhaArquivo);
		List<EventoColetado> ev = new ArrayList<EventoColetado>();
		
		if (linha.isLinhaNoPadraoEsperado()) {

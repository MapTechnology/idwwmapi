...
public class WatcherAoiKYZA {
	...
	public WatcherAoiKYZA(IcUpDTO icup, Path dir, IdwLogger log, int idLog, TratadorArquivos tratador) {
		super();
	...
	public static void main(String[] args) {
		...
		Path dir = (new File("\\\\localhost\\Folder_Lan\\Report\\Linha09")).toPath();
		...
		WatcherAoiKYZA rn = new WatcherAoiKYZA(icup, dir, log, 0, tratador);
	...
	public void avaliarDiretorios() {
		...
		if (dir.toFile().exists()) {
			...
			log.info(idLog, 0, "WatcherAoiKYZA.Diretorio : " + dir.toString() + " Não existe. Pausando a thread por 20 segundos.");
			...
			if (this.isEnviouEmail == false && emails.isEmpty() == false && icup.getIc().getEmailAoiNC().trim().equals("") == false) {
				...
				try {
					...
					e.printStackTrace();
					log.info(idLog, 0, "EnviarEmail em WatcherAoiKYZA", e);
					UtilsThreads.pausaNaThread(100);
				}
			}

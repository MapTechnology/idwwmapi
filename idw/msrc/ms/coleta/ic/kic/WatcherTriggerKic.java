...
public class WatcherTriggerKic extends AbstractWatcherTrigger {

	private BufferedEventos bufferEventos;

	@Override
	protected boolean isArquivoValido(String arquivo) {
		return arquivo.toString().toUpperCase().contains(".TXT") && arquivo.toString().toUpperCase().contains("KPLOG");
	}
	...
	protected void tratarArquivo(Path destino) {
		...
		TrataArquivoRNKic rn = new TrataArquivoRNKic(

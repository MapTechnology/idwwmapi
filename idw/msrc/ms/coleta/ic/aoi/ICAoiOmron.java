...
public class ICAoiOmron implements IIC {
	...
	private Map<String, WatcherAoiOmron> threadColetaLogs = new HashMap<>();
	
	public ICAoiOmron (IcDTO icdto) {
		super();
		this.icdto = icdto;
	}
	...
	private void inicializarOuFinalizarWatcher() {
		...
		for (IcUpDTO icup : icdto.getMsIcUpDTOLocais()) {
			WatcherAoiOmron  w = null;
			if (threadColetaLogs.containsKey(icup.getUrlConexao()) == false) {
				try {
					TratadorArquivos tratador = new TrataArquivosAoiOmronGerandoEventos(log, icdto.getMsIcUpDTOLocais(), this);
					w = new WatcherAoiOmron(icup, Paths.get(icup.getUrlConexao()), log, idLog, tratador);
		...
		for (String url : threadColetaLogs.keySet()) {
			...
			if (isExiste == false) {
				WatcherAoiOmron w = threadColetaLogs.get(url);
				w.finalizar();
		...
		if (dthrUltimaColeta == null || DataHoraRN.getQuantidadeSegundosNoPeriodo(dthrUltimaColeta, DataHoraRN.getDataHoraAtual()) > 5) {
			for (IcUpDTO icup : icdto.getMsIcUpDTOLocais()) {
				WatcherAoiOmron w = null;
				log.info(idLog, 0, "ICAoiOmron de " + icup.getUpDTO().getCd_up() + " no dir " + icup.getUrlConexao());

...
public class ServicoMaquinaOffLine implements IServico{
	...
	public MsEvt executaServico(Session sessao, MensagemRecebida mensagem) throws ServicoFalhouException {
		...
		log.iniciaAvaliacao(idLog, "Chamando ServicoMaquinaOffLine");
		
		UpRN upRN = new UpRN();
		try {
			upRN.iniciaConexaoBanco(sessao);
			upRN.lancaMaquinaOffLine(mensagem.getEventoColetado());
		} catch (Exception e){
			log.info("MedTemp: Ocorreu excessao abaixo", e);
		} finally {

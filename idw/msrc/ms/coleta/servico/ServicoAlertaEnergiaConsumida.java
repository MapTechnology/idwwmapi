...
public class ServicoAlertaEnergiaConsumida implements IServico {
	...
	public MsEvt executaServico(Session sessao, MensagemRecebida mensagem) throws ServicoFalhouException {
		...
		log.iniciaAvaliacao("Servico enviaEmailAlertaEnergiaConsumida " + mensagem.getEventoColetado().getParametroLido()+"�C");

		log.info(idLog, identacao, "Servi�o enviaEmailAlertaEnergiaConsumida");		
		...
		( (ServicoEmailAlertaParametroRN) email).setTpParametroAferido(ServicoFactory._ALERTA_CONSUMO_ATIVO);
		...
		log.paraAvaliacao();
		log.info(idLog, identacao, "Servico enviaEmailAlertaEnergiaConsumida - FIM  " + log.getAvaliacaoCompleta() );

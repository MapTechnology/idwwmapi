...
public class OmMapaDaoTest {
	...
	public void testeDao(){
		...
		Assert.assertEquals(omMapa.getIdMapa(), 1l);
		
		LeiturasCODTO leituras = new LeiturasCODTO();
		MapaCODTO map = new MapaCODTO();
		map.setCdMapa(omMapa.getCdMapa());
		leituras.setMapa(map);
		
		OmMapaDAO dao = new OmMapaDAO(daoGenericoTest.getSession());
		
		List<OmMapa> lista = dao.setCorrente(leituras, omPt);
